...
# Script tout-en-un pour corriger l'installation Proxmox (V2)
# - Corrige l'erreur 500 du redémarrage des services
# - Génère le manuel utilisateur complet
# Version 2 : Compatible avec l'environnement Proxmox actuel

echo "🚀 Correction complète de l'installation GMAO Iris (Proxmox v2)"
echo "================================================================"
...
if [ "$EUID" -ne 0 ]; then 
    ...
    echo "Utilisez: sudo bash fix_all_proxmox_v2.sh"
    exit 1
fi
...
# Redémarrer le backend en tuant le processus Python
echo -e "${YELLOW}🔄 Redémarrage du backend...${NC}"

# Trouver le PID du processus uvicorn backend
...
if [ -n "$BACKEND_PID" ]; then
    ...
    if ps -p $BACKEND_PID > /dev/null 2>&1; then
        echo "  Processus toujours actif, KILL forcé"
        kill -9 $BACKEND_PID
        sleep 1
    fi
...
# Vérifier le statut
if supervisorctl status backend 2>&1 | grep -q RUNNING; then
    echo -e "${GREEN}✅ Backend redémarré${NC}"
else
    echo -e "${YELLOW}⚠️ Vérification du statut...${NC}"
    # Vérifier si le processus tourne même si supervisorctl ne le voit pas
    if ps aux | grep -q "[u]vicorn server:app.*8001"; then
        echo -e "${GREEN}✅ Backend est actif (processus trouvé)${NC}"
    else
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${YELLOW}⚠️ Erreur possible lors de la génération${NC}"
    echo "Le manuel pourrait déjà exister. Vérifiez dans l'interface."
fi
...
echo "  ✅ Manuel utilisateur traité"
echo ""
echo "💡 Prochaines étapes:"
echo "  1. Vérifiez que le backend répond :"
echo "     ps aux | grep uvicorn"
echo ""
echo "  2. Rafraîchissez votre navigateur (Ctrl + F5)"
echo ""
echo "  3. Testez le redémarrage des services depuis l'interface"
echo ""
echo "  4. Ouvrez le manuel utilisateur pour vérifier le contenu"
